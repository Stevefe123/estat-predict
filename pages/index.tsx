...
const PredictionsDashboard = () => {
    ...
    const changeDate = (days) => setDate(currentDate => addDays(currentDate, days));
    ...
    <div className="text-center text-gray-300 py-10">
        <p className="text-xl">Performing Deep Analysis...</p>
        <p className="text-sm text-gray-500 mt-2">Checking H2H and form for the best low-scoring games. This may take a moment.</p>
    </div>
)}
            ...
            <div className="space-y-4">
                {games.map((game: any) => (
                    <div key={game.id} className="bg-gray-800 border border-gray-700 rounded-xl p-5 shadow-lg">
                        ...
                        {game.score ? (
                            <div className="text-center text-3xl font-bold my-3 text-white">
                                <span>{game.score.home} - {game.score.away}</span>
                                <p className="text-sm font-normal text-gray-500">Full-Time</p>
                            </div>
                        ) : (
                            game.weakerTeam && <p className="text-red-500 font-bold mt-2">({game.weakerTeam} - Excluded number of goals - 3)</p>
                        )}
...
const AuthPage = () => { /* ... Paste the FULL AuthPage component from the previous step here ... */ };
const PaywallPage = ({ user }) => { /* ... Paste the FULL PaywallPage component from the previous step here ... */};

// For completeness, here are the full Auth and Paywall components again.
const FullAuthPage = () => {
...
export default function Home() {
        ...
        if (session) {
            ...
            supabase.from('profiles').select('*').eq('id', session.user.id).single()
                .then(({ data, error }) => {
                    ...
                    setProfile(data);
                    setLoading(false); // Stop loading ONLY after profile is fetched
                });
        } else {
            // If there is no session, we are definitely logged out.
            setLoading(false);
        }
    }, [session, supabase]);
    ...
    // The main logic to decide what to show
    const renderContent = () => {
        // --- NEW: The core fix is here ---
        // While the initial session is being determined, show a full-page loader.
        if (loading) {
            return <div className="text-center text-gray-300 py-20">Loading...</div>;
        }

        // After loading, if there's still no user, show the public pages.
        if (!user) {
            return (
                <>
        ...
        // If we are here, the user is logged in. Now check their profile/subscription.
