...
// --- NEW: Live Score Ticker Component ---
const LiveScoreTicker = () => {
    const [liveGames, setLiveGames] = useState<any[]>([]);

    useEffect(() => {
        const fetchLiveScores = () => {
            axios.get('/api/get-live-scores').then(res => {
                setLiveGames(res.data);
            }).catch(err => console.error("Live score fetch error:", err));
        };
        fetchLiveScores(); // Fetch on initial load
        const interval = setInterval(fetchLiveScores, 60000); // Refresh every 60 seconds
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-sm z-30 overflow-hidden h-12 flex items-center">
            <div className="flex animate-marquee whitespace-nowrap">
                {liveGames.map(game => (
                    <div key={game.id} className="mx-4 text-sm">
                        <span className="text-red-500 font-bold">LIVE</span>
                        <span className="text-gray-400 mx-2">{game.teams.home.name}</span>
                        <span className="text-white font-bold">{game.goals.home} - {game.goals.away}</span>
                        <span className="text-gray-400 mx-2">{game.teams.away.name}</span>
                        <span className="text-yellow-400">({game.fixture.status.elapsed}')</span>
                    </div>
                ))}
            </div>
            <style jsx>{`
...
export default function Home() {
    ...
    useEffect(() => { if (session) { supabase.from('profiles').select('*').eq('id', session.user.id).single().then(({ data }) => { setProfile(data); setLoading(false); }); } else { setLoading(false); } }, [session, supabase]);
    ...
    const LoggedOutView = () => ( <> <LandingPage onGetStartedClick={() => { document.getElementById('auth-section')?.scrollIntoView({ behavior: 'smooth' }); }} /> <div id="auth-section"><AuthPage /></div> </> );

    return ( <div className="min-h-screen bg-gray-900 text-white font-sans pb-12"> <Header onLogout={handleLogout} isLoggedIn={!!user} /> <main> {loading && <div className="text-center text-gray-300 py-20">Loading...</div>} {!loading && (user ? <LoggedInView /> : <LoggedOutView />)} </main> <WhatsAppFooter /> {!user && <DisclaimerFooter />} </div> );
}